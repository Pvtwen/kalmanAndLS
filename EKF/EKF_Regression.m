...
for kk=1:100
    ...
    X_ekf=zeros(4,N);% init  estimation
    ...
    for k=2:N
        ...
        % linearization
        H_tilta=[I;H];
        Y_tilta=[Xekf_pre;Z(:,k)-Zekf_pre+H*Xekf_pre];
        R_tilta=[P_pre zeros(4,2);zeros(2,4) R];

        %normalization
        St=R_tilta^(1/2);
        H_tilta1=St^(-1)*H_tilta;
        Y_tilta1=St^(-1)*Y_tilta;
        
        X_ekf(:,k)=(H_tilta1'*H_tilta1)^(-1)*H_tilta1'*Y_tilta1;
...
legend('real orbit','EKF orbit','Observation orbit');
